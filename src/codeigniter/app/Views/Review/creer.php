<!DOCTYPE html>
<html lang="fr">
<?= view('components/head', ['title' => ($title ?? 'Laisser un avis') . ' - OnlyShoes']) ?>
<body class="bg-gray-50">
    ...
    <main class="max-w-3xl mx-auto px-4 sm:px-6 py-10">
        <div class="bg-white rounded-3xl shadow-sm p-8">
            ...
            <form action="<?= base_url('review/store') ?>" method="POST" class="space-y-6">
                <?= csrf_field() ?>
                <input type="hidden" name="id_vendeur" value="<?= esc($vendeur['id_utilisateur']) ?>">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2" id="star-rating">
                        <?php for ($i = 1; $i <= 5; $i++): ?>
                            <input type="radio" name="note" value="<?= $i ?>" id="star-<?= $i ?>" class="sr-only" <?= $i === 1 ? 'checked' : '' ?> required>
                    ...
                    <p class="text-xs text-gray-500 mt-2">Cliquez pour sélectionner une note de 1 à 5 étoiles</p>
                </div>
                ...
                <div>
                    ...
                    <textarea id="commentaire"
                              ...
                              placeholder="Partagez votre expérience avec ce vendeur..."></textarea>
                    <p class="text-xs text-gray-500 mt-2">
                        Décrivez la qualité du produit, la rapidité d'expédition, la communication, etc.
                    </p>
                </div>
