...
composer_install() {
    ...
    if $SUDO docker exec sae_web test -f "/src/codeigniter/vendor/autoload.php" 2>/dev/null; then
...
menu() {
    echo "Docker - Projet SAE (docker compose)"
    echo ""
    echo "Commands:"
...
nettoyer() {
    ...
    if [ "$confirm" = "oui" ]; then
        $SUDO $COMPOSE_CMD --env-file $ENV_FILE -f $COMPOSE_FILE down -v
        $SUDO docker volume rm docker_postgres-data docker_embeddings-cache 2>/dev/null || true
        echo "Cleanup done"
    else
        echo "Canceled"
    fi
...
fix_permissions() {
    ...
    # On Mac, don't use sudo for local files
    if [ "$(uname -s)" = "Darwin" ]; then
        ...
        sudo chmod +x docker.sh podman.sh 2>/dev/null || true
    fi
    ...
    if $SUDO docker ps --format '{{.Names}}' 2>/dev/null | grep -q "^sae_web$"; then
        if ! $SUDO docker exec sae_web chmod -R 777 /src/codeigniter/writable 2>/dev/null; then
            echo "chmod failed in container"
            exit 1
        fi
        # chown will fail with volumes, so make it optional
        $SUDO docker exec sae_web chown -R www-data:www-data /src/codeigniter/writable 2>/dev/null || true
    else
        echo "Container sae_web not running"
        echo "Start with: ./docker.sh demarrer"
        exit 1
    fi
...
container_running() {
    local name="$1"
    $SUDO docker ps --format '{{.Names}}' 2>/dev/null | grep -q "^${name}$"
}
...
run_sql() {
    if [ -z "$2" ]; then
        echo "Usage: ./docker.sh sql <file.sql>"
        exit 1
    fi
